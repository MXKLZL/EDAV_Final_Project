...
We have randomly sampled 1000 data to analysis the missing value pattern of the dataset after we have cleaned the data. We have also changed the "UNKNOWN" entry to NA.
```{r echo=FALSE}
...
We can further investigate the missing row patterns by using the missing_data.frame function.
```{r include=FALSE}
...
We have found out that there are 30 different row missing patterns. Some of the patterns only have one or two occurrences and 20 of them have 10 occurrences or below.

We can check how many times each of the row missing pattern appears in the data set by the following and also visualize these row missing patterns with visna function.

```{r fig.height = 4, echo=FALSE}
visna(df, sort='r')
```
(Note that in the above plot, the columns represent the seven variables and the rows the missing patterns. The cells for the variables with missing values in ta pattern are drawn in blue.)

By using the visna function, we observed the following missing row patterns:

1. The pattern that occurs the most is missing both Latitude and Longitude, which is about 92% of the data.
2. The information on the suspect is missing such as age, sex, and race tends to be missing along with some other variables.

Since the objective of our project is to analyze the safety issue, when doing the interactive part of the project, we will need to use the location information. So we have to drop all the data with Longitude and Latitude missing. This might affect the distribution of the crime, so we plotted the following diagram to compare the distribution of crime type before and after the missing location data is removed.
